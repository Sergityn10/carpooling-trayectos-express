...
async function requestJson(path, { query } = {}) {
  if (!apiOil) {
    throw new Error(
      "API_OILPRICE no está configurada en las variables de entorno",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Error consultando Precioil.es (${response.status}): ${await response.text()}`,
    );
  }
...
function findProvinceByName(provincia) {
  ...
  const match = provinces.find(
    (p) => normalizeProvinceName(p.nombreProvincia) === normalizedInput,
  );
  if (!match) {
...
async function getEstacionHistorico(
  idEstacion,
  { fechaInicio, fechaFin } = {},
) {
  return requestJson(`/estaciones/historico/${idEstacion}`, {
...
async function getEstacionesRadio({
  latitud,
  longitud,
  radio,
  pagina,
...
async function getPrecioMedioDiario({
  idFuelType,
  fechaInicio,
  fechaFin,
} = {}) {
...
async function getPreciosMediosByProvinciaId(idProvincia, idFuelType) {
  return requestJson(`/precios/medios/provincia/${idProvincia}`, {
    query: { idFuelType },
  });
}
...
async function getGasoilAveragePriceByProvinciaId(idProvincia) {
  ...
  if (!item || typeof item.averagePrice !== "number") {
    throw new Error(
      `No se pudo obtener el precio medio de gasoil para la provincia ${idProvincia}`,
    );
  }
